...
<section class="bg-[#FAF9F7] py-16 md:py-20">
	<div class="max-w-[1200px] mx-auto px-4">
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
			...
			?>
			<a href="<?php echo esc_url( guardexpert_get_category_url( $category ) ); ?>" class="bg-white rounded-[2px] overflow-hidden shadow-md hover:shadow-xl transition-shadow group">
				<div class="md:aspect-square p-0 flex items-center justify-center">
					<?php if ( $image_url ) : ?>
						<img src="<?php echo esc_url( $image_url[0] ); ?>" alt="<?php echo esc_attr( $category->name ); ?>" class="w-full h-[140px] md:h-full object-contain">
					<?php else : ?>
						<img src="<?php echo get_template_directory_uri(); ?>/img/cat-placeholder.png" alt="<?php echo esc_attr( $category->name ); ?>" class="w-full h-[140px] md:h-full object-contain">
					<?php endif; ?>
				</div>
				<div class="p-2">
					<h3 class="text-[22px] font-semibold text-black mb-2 group-hover:text-[#B3262E] transition-colors">
						<?php echo esc_html( $category->name ); ?>
					</h3>
					<div class="flex items-center gap-2 group">
						<p class="text-base text-black mb-3 max-w-[94%]">
